...
| :--- | :--- | :--- | :--- |
| [TAEs001.R](TAEs001.R) | Regressão Linear Múltipla | Eliminação Regressiva (Backward Elimination) manual e automática (`step`) | `Carseats` (`ISLR`/`ISLR2`) |
| [TAEs002.R](TAEs002.R) | Predição e Formulação Matemática | Construção manual da equação de predição com os coeficientes estimados vs `predict()` do R | `Auto`, `Carseats`, `Credit` |
| [TAEs003.R](TAEs003.R) | Seleção de Atributos (Feature Selection) | Stepwise: Forward, Backward e Both (Ambos os sentidos) | `mtcars`, `Auto`, `Carseats`, `Hitters` |
| [TAEs004.R](TAEs004.R) | Classificação Binária | Regressão Logística, cálculo manual de Acurácia, Sensibilidade e Especificidade | `Smarket`, `Default`, `Weekly`, `Caravan` |
| [TAEs005.R](TAEs005.R) | Otimização de Limiares (Thresholds) | Curvas ROC, cálculo de AUC e busca pelo limiar ótimo via Youden's Index | `Smarket` (`ISLR`/`ISLR2`) |
| [TAEs006.R](TAEs006.R) | Script Mestre / Automação | Funções reutilizáveis para automação de Stepwise e otimização ROC em lotes | `mtcars`, `Auto`, `Carseats`, `Hitters`, `Default`, `Weekly`, `Caravan` |
| [TAEs007.R](TAEs007.R) | Modelagem com Dados Externos | Importação, limpeza e escala corretiva de dados em arquivos CSV personalizados | `autos.csv`, `boston_housing.csv`, `heart_disease_uci.csv` |
| [TAEs008.R](TAEs008.R) | Modelos Não-Lineares & Multiclasse | Árvores de Regressão e Classificação com Poda por Cp (Cost-Complexity) e Regressão Logística Multinomial | `diamonds`, `Sonar`, `penguins` |
